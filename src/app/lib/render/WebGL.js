...
const getShader = function( type, shaderImport) {
    ...
    self.gl.shaderSource(shader, shaderImport);
...
const initGl = function(){
    const self = _self.get(this);
    self.gl = self.canvas.getContext("experimental-webgl", { antialias: true });
    ...
    constructor(showFPS=false) {
        ...
        if (showFPS) {
            ...
            window.setInterval(function(){
                document.getElementById('fps').innerHTML = self.fps;
                self.fps = 0;
            },1000);
        }
